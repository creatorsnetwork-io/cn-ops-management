...
export default async function Page({ params }) {
  let p = null;
  try { p = await sanity(true).fetch('*[_type=="project" && slug==$s][0]{slug,name,type,"client":client->name}', { s: params.slug }); } catch (e) {}
  ...
  return (
    <>
      ...
      <ProjectTabs slug={p.slug} type={p.type} on="/count" />
