...
const money = (n) => (n ? 'AED ' + Number(n).toLocaleString('en-GB') : '—');
    ...
    <>
      <div className="panel">
        ...
        <div style={{ padding: '14px 16px' }}>
          <div className="stat">
            <div><b>{live.length}</b><span>live</span></div>
            <div><b>{money(weighted)}</b><span>per month if all landed</span></div>
            <div><b style={{ color: overdue ? 'var(--bad)' : undefined }}>{overdue}</b><span>next step overdue</span></div>
            <div><b style={{ color: 'var(--ok)' }}>{closed.filter((p) => p.stage === 'won').length}</b><span>won</span></div>
          </div>
      ...
      {adding ? (
        <div className="panel">
          ...
          <div style={{ padding: '14px 16px', display: 'grid', gap: 11, gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))' }}>
            <label><div className="k">Who</div><input className="inp" value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} /></label>
            <label><div className="k">Market</div>
              <select className="inp" value={f.market} onChange={(e) => setF({ ...f, market: e.target.value })}>
                {['Dubai', 'GCC', 'India', 'Europe', 'Other'].map((m) => <option key={m}>{m}</option>)}
              </select></label>
            <label><div className="k">What they might buy</div><input className="inp" value={f.service} onChange={(e) => setF({ ...f, service: e.target.value })} /></label>
            <label><div className="k">Monthly value, AED</div><input className="inp" value={f.valueAed} onChange={(e) => setF({ ...f, valueAed: e.target.value })} /></label>
            <label><div className="k">Where they came from</div><input className="inp" value={f.source} onChange={(e) => setF({ ...f, source: e.target.value })} /></label>
            <label><div className="k">Next step</div><input className="inp" value={f.nextStep} onChange={(e) => setF({ ...f, nextStep: e.target.value })} /></label>
            <label><div className="k">By when</div><input className="inp" type="date" value={f.nextStepDate} onChange={(e) => setF({ ...f, nextStepDate: e.target.value })} /></label>
          </div>
      ...
      <div className="tabs">
        <button className={tab === 'live' ? 'on' : ''} onClick={() => setTab('live')}>Live ({live.length})</button>
        <button className={tab === 'closed' ? 'on' : ''} onClick={() => setTab('closed')}>Won and lost ({closed.length})</button>
      </div>
      ...
      <div className="panel" style={{ marginTop: 0, borderTopLeftRadius: 0, borderTopRightRadius: 0 }}>
                  ...
                  <Fragment key={p._id}>
                    <tr>
                      ...
                      <td>{p.nextStep || <span style={{ color: 'var(--faint)' }}>nothing planned</span>}
                        {p.nextStepDate ? <div className="mono" style={{ fontSize: 12, color: p.nextStepDate < today() ? 'var(--bad)' : 'var(--faint)' }}>
                    ...
                    {ask && ask.p._id === p._id ? (
                      <tr><td colSpan={6} style={{ background: '#FCFDFE' }}>
                        <label style={{ display: 'block' }}>
                          <div className="k">{ask.stage === 'won' ? 'What did they sign, and at what' : 'Why it was lost'}</div>
        ...
        {err && !d ? <div className="row"><span className="dot no" /><div className="t"><b>Could not load the pipeline</b><span className="err">{err}</span></div></div> : null}
      </div>
      <p className="note">
