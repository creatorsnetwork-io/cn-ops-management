...
function Email({ slug, initial, canEdit, domain }) {
  ...
  if (!canEdit) return <span className="dim">{v || '—'}</span>;
...
export default function Team({ rows, canEdit, domain }) {
  ...
  return (
    <>
      ...
      <div className="panel">
        <header><h2>Who is carrying what</h2><span className="pill">{rows.filter((r) => r.active).length} active</span></header>
        <table className="tbl">
          <thead><tr>
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={p.slug} style={{ opacity: p.active ? 1 : 0.5 }}>
                ...
                <td>{p.reportsToName || '—'}</td>
                <td>{p.open ? <b>{p.open}</b> : '—'}</td>
                <td>{p.late ? <span className="tag bad">{p.late}</span> : '—'}</td>
                <td>{p.queue ? <span className="tag warn">{p.queue}</span> : '—'}</td>
