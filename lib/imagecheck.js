...
export function toNotes(item, result) {
  if (!result) return [];
  ...
  for (const s of result.slides || []) {
    ...
    for (const t of s.textIssues || []) {
      // quoted/correction only ever reach here once already different, so this
      // always names a real change, not a restated non-issue.
      bits.push('"' + t.quoted + '" should be "' + t.correction + '"' + (t.why ? ' (' + t.why + ')' : '') + '.');
    }
    if (s.unclearNote) bits.push(s.unclearNote);
