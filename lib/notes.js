...
// A meeting has a kind and, separately, things it concerns. An internal call about
// STCH is internal AND about STCH. Treating those as one field was the mistake.
export const KINDS = {
  client:   { label: 'With the client', needs: 'client' },
  internal: { label: 'Internal', needs: '' },
  vendor:   { label: 'Vendor or freelancer', needs: 'vendor' },
  prospect: { label: 'Prospect', needs: 'prospect' },
};

// Older records only had a true or false internal flag. Read them without a migration.
export function kindOf(n) {
  if (n && n.kind && KINDS[n.kind]) return n.kind;
  if (n && n.internal) return 'internal';
  if (n && n.clientSlug) return 'client';
  return '';
}

// Whether there is enough on a note to read it and draft a recap.
export function readyToDraft(n) {
  const k = kindOf(n);
  if (!k) return { ok: false, why: 'Say what kind of meeting this was first.' };
  if (k === 'client' && !n.clientSlug) return { ok: false, why: 'Pick the client.' };
  if (k === 'vendor' && !n.vendorId) return { ok: false, why: 'Pick the vendor or freelancer.' };
  if (k === 'prospect' && !n.prospectId) return { ok: false, why: 'Pick the prospect from the pipeline.' };
  return { ok: true };
}
...
// Gemini lists the attendees near the top of every notes document. The email
// addresses are the reliable part, so those are read here and the names are left
// to the model, which handles "Himanshu Arora Priyanka Sahai" better than a regex.
const OUR_DOMAINS = ['creatorsnetwork.io', 'gmail.com', 'googlemail.com'];

export function emailsIn(text) {
  const head = String(text || '').slice(0, 3000);
  const found = head.match(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/g) || [];
  const seen = new Set();
  return found.map((e) => e.toLowerCase()).filter((e) => (seen.has(e) ? false : (seen.add(e), true))).slice(0, 30);
}

export function outsideDomains(emails) {
  const seen = new Set();
  for (const e of emails || []) {
    const d = e.split('@')[1];
    if (d && !OUR_DOMAINS.includes(d)) seen.add(d);
  }
  return [...seen];
}

// Who was in the room, matched against people we already have on record. Used to
// suggest a client or a prospect when the meeting title gives nothing away.
export function suggestFrom(emails, clients, prospects) {
  const domains = outsideDomains(emails);
  if (!domains.length) return { kind: 'internal', why: 'Everyone in the meeting was on a CN address.' };

  for (const c of clients || []) {
    for (const ct of c.contacts || []) {
      const d = String(ct.email || '').toLowerCase().split('@')[1];
      if (d && domains.includes(d))
        return { kind: 'client', clientSlug: c.slug, why: 'Someone from ' + d + ' was in the meeting, and that is ' + c.name + '.' };
    }
  }

  for (const p of prospects || []) {
    const words = String(p.name || '').split(/\s+/).filter((w) => w.length > 3);
    if (words.some((w) => domains.some((d) => d.includes(squash(w)))))
      return { kind: 'prospect', prospectId: p._id, why: 'The domain looks like ' + p.name + ', which is in the pipeline.' };
  }

  return { kind: '', why: 'Outside people from ' + domains.join(', ') + ', but none match a client or a prospect on record.' };
}
...
Return JSON only, shaped exactly:
{
  ...
  "attendees":[{"name": "the person", "side": "us or them or unknown"}],
...
- "attendees" comes from the participant list near the top of the notes. Copy the names as written. "side" is us for anyone from Creators Network and them for anyone from the client or an outside company. Use unknown rather than guessing.`;
...
export async function recapFrom(text, clientName) {
  ...
  out.attendees = (Array.isArray(parsed.attendees) ? parsed.attendees : [])
    .map((x) => ({
      name: String((x && x.name) || '').trim().slice(0, 80),
      side: ['us', 'them'].includes((x && x.side) || '') ? x.side : 'unknown',
    }))
