...
export const CHECK_PROMPT_VERSION = 5;
...
export async function checkSlide({ imageBuffer, caption, embeddedText }) {
  ...
  const instructions =
    ...
    + '3. Read any text visible on the slide itself. List only genuine mistakes: a misspelling, a cut-off word, a missing word, or grammar that is actually broken. '
    ...
    + 'For each genuine mistake, give the exact phrase precisely as it appears on the slide, character for character, and separately the corrected version of that same phrase. '
    ...
    + 'If any text is too small or unclear for you to read with confidence, put one plain sentence saying so in unclearNote instead of guessing a correction, and do not invent a textIssue for it.\n'
    ...
    + '"textIssues": [{"quoted": "the exact phrase as it appears on the slide", "correction": "that same phrase, corrected", "why": "a few words naming the kind of mistake"}], '
  ...
  const text = ((j.choices || [])[0] || {}).message?.content || '{}';
  let out;
  try { out = JSON.parse(text); } catch (e) { out = { placeMatch: 'unclear', whatItShows: '', textIssues: [], unclearNote: '' }; }
  ...
  // claimed it needed capitalizing). Trusting the prompt alone to prevent
  // that is not enough, so anything where the quoted text and the
  // correction are the same words is dropped here regardless of what the
  // model's own wording claims.
  ...
  const textIssues = (Array.isArray(out.textIssues) ? out.textIssues : [])
    ...
    .filter((t) => t.quoted && t.correction && !same(t.quoted, t.correction))
    .slice(0, 8);
