...
// A brand brain field counts once it holds anything at all. Whether that came
// from typing it in or from an imported document does not matter here.
function hasBrandBrain(b) {
  if (!b) return false;
...
export function evaluate(c) {
  ...
  // Brand lives on the client now (lib/brand.js), not per project. The old
  // free-text voice note is still read too, so a client set up before that
  // existed does not fall back to looking unfinished.
  const brandDone = hasBrandBrain(c.brand) || projects.some((p) => p.voice && p.voice.trim());
  ...
  const why = [
    ...
    derived[2] ? (hasBrandBrain(c.brand) ? 'Brand brain filled in.' : 'A voice note is written.') : 'No brand brain and no voice note yet, so drafts come back deliberately plain.',
